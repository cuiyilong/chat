...
INT32 chat_client_comm_init(void)
{
    #if 0
    ...
    while(1)
    {
        ...
        LOG(LOG_OFF,"udp recv data  : addr :"IP_FMT" port %d datalen %d data %s! ",IP_ARG((INT8 *)&addr_v4.sin_addr),addr_v4.sin_port,datalen,aucBuf);
    }
    #endif
}
